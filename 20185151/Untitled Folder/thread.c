...
int calc_priority(int recent_cpu, int nice) {
  ...
  if (result > convert_to_fixed_point(PRI_MAX)) {
    result = convert_to_fixed_point(PRI_MAX);
  }
  else if (result < convert_to_fixed_point(PRI_MIN)) {
    result = convert_to_fixed_point(PRI_MIN);
  }
  return result;
}
...
{
  intr_disable();  //  threads/interrupt.h에 정의                                         
...
{
  intr_disable();    
  // return convert_to_int_to_nearest(multiply_fixed_and_int(load_avg, 100));       
